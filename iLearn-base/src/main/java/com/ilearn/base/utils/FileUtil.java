...
public class FileUtil {
     ...
     * 读取文件内容，作为字符串返回
     */
    public static @NotNull String readFileAsString(String filePath) throws IOException {
     ...
     * 根据文件路径读取byte[] 数组
     */
    public static byte @NotNull [] readFileByBytes(String filePath) throws IOException {
        File file = new File(filePath);
        if (!file.exists()) {
            throw new FileNotFoundException(filePath);
        } else {
